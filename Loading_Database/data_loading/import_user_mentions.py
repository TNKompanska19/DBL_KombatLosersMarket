...
def main():
    ...
    for rec in all_records:
        ...
        for m in rec.get("entities", {}).get("user_mentions", []):
            mentions.append({
                "tweet_id": tweet_id,
                "user_id": m.get("id"),
    ...
    if not mentions:
        print("No user_mentions found in any tweet.")
        return

    df = pd.DataFrame(mentions, columns=["tweet_id", "user_id", "screen_name", "indices"])

    # sanitize column names for Postgres:
